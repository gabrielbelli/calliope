"""Wake words and the end of the command that follows one.
    ...
    ww.clone(["alexa"])                      another stream, on the same sessions
    ep = Endpointer()
...
classes hold per-stream state, so a hub with several satellites makes one of
each per satellite. Nothing here talks to a satellite: no lights, no speaker.
...
log = logging.getLogger("voice-satellites.wakeword")
...
class WakeWords:
    ...
    def _warm_up(self) -> int:
        ...
        return (max(self._model.model_inputs[k] for k in self._keys.values()) - 1) * FRAME

    @property
    def names(self) -> list[str]:
        """The wake words this stream listens for."""
    ...
    def clone(self, names: Iterable[str] | None = None) -> WakeWords:
        """A second stream on the models this one loaded, for another
        satellite: on all of them, or only on `names`, the words assigned to
        that satellite.
        ...
        A copy on a subset runs only those sessions on its frames, because
        Model.predict runs every session in Model.models: a word assigned to
        the kitchen costs the bedroom nothing and cannot wake it.

        The thresholds dict is shared as well, so a change reaches every
        satellite without a new stream. This leans on openwakeword 0.6.0's
        internals (Model.models, Model.preprocessor and its raw_data_buffer);
        the version is pinned, and tests feed a clone, a clone on a subset and
        fresh instances the same audio and compare every score.
        """
        twin = copy.copy(self)
        model = copy.copy(self._model)
        if names is not None:
            names = list(names)
            unknown = [n for n in names if n not in self._keys]
            if not names or unknown:
                raise ValueError(f"cannot listen for {names} on a stream that loaded "
                                 f"{list(self._keys)}")
            twin._keys = {n: self._keys[n] for n in names}
            model.models = {key: self._model.models[key] for key in twin._keys.values()}
            # A shorter window, if the longest model is not among these.
            twin._warmup = twin._warm_up()
        features = copy.copy(self._model.preprocessor)
        # reset() clears this deque in place rather than replacing it, so a
        # shared one would let one satellite's reset wipe another satellite's
        # audio.
...
class Endpointer:
    ...
    FRAME_MS = 20        # the satellite's own frame length
    ...
    def __init__(self, rate: int = RATE, max_seconds: float = 10.0, silence_ms: int = 800,
        ...
        # preroll(): audio from before the wake word fired. Kept, never a start.
        self._live = 0                      # first frame fed through feed()
        self._pre_start: int | None = None  # where speech began inside the preroll
    ...
    def had_speech(self) -> bool:
        return self._start is not None or bool(self._held)
    ...
    def audio(self) -> bytes:
        ...
        return self._held + bytes(self._pcm[first * self._n * 2:last * self._n * 2])

    # Speech voiced within this many frames of the preroll's end is the same
    # breath as what follows: "hey jarvis, what time is it" said straight
    # through. Anything earlier is the wake word, then a pause.
    CONTINUOUS = 10      # frames: 200 ms
    # Speech that crosses the detection point and stops within this many frames
    ...
    def preroll(self, pcm: np.ndarray) -> None:
        ...
        THE REWOUND AUDIO MUST NOT START THE COMMAND, and the live board said
        ...
        So this audio is kept (the one-breath case needs it) and webrtcvad
        hears it (so it has adapted), but a command starts only on speech
        after it -- unless that speech was already running when it ended, in
        which case the command starts where it began here.
        """
        x = _mono16(pcm)
        n = self._n
        whole = len(x) - len(x) % n
        for off in range(0, whole, n):
            ...
            if voiced:
                self._voiced = i
                self._pre_last = i
            if sum(self._recent) >= self.ONSET_VOICED and self._pre_start is None:
                self._pre_start = i - len(self._recent) + 1 + self._recent.index(True)
        self._live = self._frames
    ...
    def feed(self, pcm: np.ndarray) -> bool:
        ...
        for off in range(0, whole, n):
            ...
            if sum(self._recent) >= self.ONSET_VOICED:
                if self._start is None:
                    onset = i - len(self._recent) + 1 + self._recent.index(True)
                    running = (self._pre_start is not None
                               and self._live - 1 - self._pre_last <= self.CONTINUOUS
                               and onset <= self._live)
                    # One breath: begin where it began in the preroll. After a
            ...
            if (self._start is not None and i - self._last_speech >= self._silence_frames
                ...
                # detection point stopped right after it, so it was the word
                ...
                self._reopened_at = i
            elif self._start is not None and i - self._last_speech >= self._silence_frames:
                self.reason = "silence"
            elif (self._start is None and self._reopened_at is not None
                    and i - self._reopened_at >= math.ceil(self.REOPEN_S * 1000 / self.FRAME_MS)):
                self.reason = "silence"      # nothing more came: the held audio is the command
            elif self._frames - self._live >= self._max_frames:
                self.reason = "max_length"
            elif (self._start is None and self._reopened_at is None
                    and self._frames - self._live >= self._timeout_frames):
