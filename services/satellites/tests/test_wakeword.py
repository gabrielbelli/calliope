...
SATELLITES_TEST_WAKEWORD_DIR if that is set, so repeated runs can skip the
fetch. Offline, every test that needs them is skipped with the reason; the
Endpointer and ensure_models tests need no network and always run.
"""
...
def feed_all(ww: WakeWords, x: np.ndarray, n: int = 320) -> list[Detection]:
    """In the satellite's own 20 ms frames unless told otherwise."""
...
def test_two_clones_on_different_satellites_do_not_hear_each_other(model_dir):
    """Each satellite's audio buffer must be its own. openWakeWord reads the
    last 1760 samples of it per 80 ms frame, so a buffer shared between two
    satellites mixes 30 ms of one into every frame of the other and moves
    every score without always stopping a detection; and a reset of one
    would clear the other's. So the talking satellite, fed alongside a
    satellite that hears only the room and is reset halfway, must score
    ...
    for i, (a, b) in enumerate(zip(chunks(phrase, 320), chunks(room_only, 320))):
        if i == 75:  # 1.5 s in: the other satellite reconnects halfway through "jarvis"
            quiet.reset()
...
# ---- the preroll: rewound audio is kept but never starts the command ------------
...
def test_a_pause_after_the_wake_word_does_not_end_the_command_before_it_starts():
    ...
    word = voiced(0.35)
    question = voiced(1.2)
    # 0.9 s of rewind: the word's tail, then the silence the detector took.
    pre = room(0.9, (0.0, word))
    # Live: a 1.0 s pause (longer than silence_ms), then the question.
    live = room(3.5, (1.0, question))
    ep = Endpointer()
    ep.preroll(pre)
    done = run(ep, live)
    assert done is not None and ep.reason == "silence" and ep.had_speech
    got = seconds(ep.audio)
    # The question, whole, with 300 ms either side and webrtcvad's ~80 ms
    # hangover -- and not the wake word, which would make it 2.9 s or more.
    assert 1.2 <= got < 2.3


def test_speech_already_running_when_the_detector_fires_keeps_its_start():
    """Said in one breath, the command began inside the rewound audio and is
    still going when the detector fires; it must start where it began."""
    sentence = voiced(1.6)
    pre = room(0.9, (0.3, sentence[:int(0.6 * RATE)]))
    live = room(2.5, (0.0, sentence[int(0.6 * RATE):]))
    ep = Endpointer()
    ep.preroll(pre)
    assert run(ep, live) is not None and ep.had_speech
    # All 1.6 s of it, from its first voiced frame in the preroll.
    assert seconds(ep.audio) >= 1.6


def test_the_start_timeout_counts_from_the_wake_word_not_from_the_rewind():
    ep = Endpointer(start_timeout_s=1.0)
    ep.preroll(room(0.9, (0.0, voiced(0.35))))
    # 0.5 s of silence after the wake word: the rewind must not have used up
    # the timeout.
    assert run(ep, floor(0.5)) is None
    assert run(ep, floor(1.0)) is not None and ep.reason == "no_speech"


def test_the_wake_word_then_a_pause_then_the_question_records_the_question():
    ...
    pre = room(0.9, (0.5, word[:int(0.4 * RATE)]))
    live = room(5.0, (0.0, word[int(0.4 * RATE):]), (1.4, question))
    ep = Endpointer()
    ep.preroll(pre)
    assert run(ep, live) is not None and ep.reason == "silence"
    # The held word and the question, both: over 2 s of audio.
    assert seconds(ep.audio) > 2.0


def test_the_wake_word_alone_is_still_answered_after_the_reopen_window():
    ep = Endpointer()
