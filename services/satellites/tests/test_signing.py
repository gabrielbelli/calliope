...
them (clients/korvo-satellite/scripts/firmware_signing.py): a signature the
upload script produces must pass here, and one the satellite would refuse must
not.
...
SCRIPTS = Path(__file__).resolve().parents[3] / "clients" / "korvo-satellite" / "scripts"
...
def test_a_hub_key_the_satellite_could_not_use_is_refused_when_loaded(tmp_path, monkeypatch):
...
def test_the_build_script_refuses_a_public_key_the_satellite_could_not_use(script):
...
def test_a_satellite_built_without_a_key_takes_signed_and_unsigned_images():
    assert signing.skip_reason({"mic": {}}, None) is None
    assert signing.skip_reason({}, "c2ln") is None


def test_a_satellite_that_requires_signatures_is_not_sent_an_unsigned_image():
    assert "unsigned" in signing.skip_reason({"ota_key": "0123456789abcdef"}, None)


def test_a_satellite_that_trusts_another_key_is_not_sent_an_image_it_would_refuse(key, hub_key):
...
def test_a_satellite_that_trusts_the_hubs_key_is_updated(key, hub_key):
