"""Earcons: short feedback sounds the hub uploads once and each node keeps in
flash, so that "I heard you", "done" and "that failed" play from the node
itself, with no audio crossing the network when they are needed.

On the wire (the node's side is clients/korvo-node/src/earcons.cpp):

    hub  -> node  {"type": "earcon_put", "id": "wake", "size": N, "sha256": "..."}
    node -> hub   {"type": "earcon_next", "id": "wake", "offset": 0}
    hub  -> node  binary: kind 4, 0, 0, 0, offset u32 LE, then up to 8 KB
          ... one chunk per earcon_next, as for firmware ...
    node -> hub   {"type": "earcon_stored", "id", "size", "sha256"}
                  or {"type": "earcon_failed", "op": "put", "id", "error"}
    hub  -> node  {"type": "earcon", "id": "wake"}        play it now
    hub  -> node  {"type": "earcon_list"}
    node -> hub   {"type": "earcons", "ready": bool, "items": [{id, size, sha256}],
                   "last_load_us": N}
    hub  -> node  {"type": "earcon_delete", "id": "wake"}  answered with "earcons"

Files are raw mono s16le at 48 kHz, at most 2 s (192 000 bytes), at most 16
per node, with ids of [a-z0-9_-]{1,24}. An earcon plays over whatever the hub
...
RATE = 48000  # the node's speaker rate; earcons are played as stored, never resampled
FRAME_EARCON = 4
...
# Amplitude before the envelope. The default set peaks at -13 to -12 dBFS, which
# leaves room to be mixed over speech on the node without clipping much.
LEVEL = 0.3
...
def check(eid: str, pcm: bytes) -> None:
    ...
    if len(pcm) > MAX_BYTES:
        raise EarconError(f"earcon {eid!r} is {len(pcm)} bytes; a node keeps at most "
...
class Sync:
    """Brings one node's earcons in line with `want`, one upload at a time, the
    only way a node takes them. It does no I/O: hand it the node's messages
    through `handle` and send whatever it returns (a dict as JSON, bytes as a
    binary frame). One per connected node; start it by sending earcon_list.
    """

    def __init__(self, want: dict[str, bytes]):
        if len(want) > MAX_EARCONS:
            raise EarconError(f"{len(want)} earcons; a node keeps at most {MAX_EARCONS}")
        ...
        # False until the node answers earcon_list with its storage mounted.
        # The first mount after a blank partition formats it in the background,
        # so ask again a few seconds later.
        self.ready = False
    ...
    def has(self, eid: str) -> bool:
        """The node holds `eid` with the content this hub wants: it listed it
        already, or stored it since. Anything else, a play would only come
        back as earcon_failed, so the hub plays nothing instead."""
    ...
    def plan(self, items: list[dict]) -> list[str]:
        """The ids the node lacks, or holds with different content."""
