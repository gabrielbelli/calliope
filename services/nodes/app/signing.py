...
only when it carries an ECDSA P-256 signature, by the matching private key,
over the image's SHA-256. The developer signs on their own machine
...
Nothing here is what keeps a node safe; the node checks for itself, so a hub
that is compromised still cannot install its own firmware. What this module
does is refuse mistakes before a 15 s transfer ends in "bad signature": a
signature that is not one, and, when NODES_FIRMWARE_PUBKEY names the key, one
made with the wrong key.

`cryptography` is imported only when NODES_FIRMWARE_PUBKEY is set, so a hub
that does not check signatures does not need it.
"""
...
def check_der(der: bytes) -> None:
    """The shape mbedTLS on the node will parse, checked without a key."""
...
def load_public_key(value: str | None = None) -> Any | None:
    """The hub's copy of the firmware key, from NODES_FIRMWARE_PUBKEY: a path to
    a PEM file, or the PEM itself. None when the variable is unset or empty."""
    ...
    if not isinstance(key, ec.EllipticCurvePublicKey) or key.curve.name != "secp256r1":
        raise SignatureError(f"{ENV_KEY} is not an ECDSA P-256 key, the only kind the node "
                             "can verify")
    return key
...
def key_id(public_key: Any) -> str:
    """The same short name the firmware build prints and the node reports as
    caps.ota_key: the first 16 hex of the DER public key's SHA-256."""
...
def accept_upload(image: bytes, signature: str | None, public_key: Any | None = None) -> str | None:
    """What POST /nodes/firmware keeps from its `signature` parameter: standard
    base64 to store beside the image and send in the "ota" message, or None for
    an unsigned image. Raises SignatureError (answer 400) for a malformed
    signature and, when the hub has a key, for a missing or wrong one."""
...
def skip_reason(caps: dict, signature: str | None, public_key: Any | None = None) -> str | None:
    """Why POST /nodes/ota should skip a node rather than start a transfer the
    node will refuse, or None to go ahead. A node that reports no `ota_key` was
    built without one and takes anything."""
    node_key = (caps or {}).get("ota_key")
    if not node_key:
        return None
    if not signature:
        return f"the node only installs images signed by key {node_key}; this one is unsigned"
    if public_key is not None and key_id(public_key) != node_key:
        return (f"the node trusts key {node_key}, but this image was checked against "
                f"key {key_id(public_key)}")
    return None
