"""Routing: what happens after a wake word.
    ...
    handle(node_id, node_name, wake_word, utterance_pcm16k) -> Outcome
      1. the first rule, in file order, whose wake word and nodes match
      2. STT   the utterance as a WAV to NODES_STT_URL/v1/audio/transcriptions
      3. the rule's destination (destinations.py) with the transcript
      4. TTS   the reply to NODES_TTS_URL/v1/audio/speech as 24 kHz pcm,
...
lights (a node with lights_enabled false is never sent "lights") out of this
module entirely: whoever plays the Outcome owns that check.
...
and leave the node deaf until a reconnect. Every failure, down to a bug in
this file, becomes Outcome.error, and every external call has a timeout.

PRECEDENCE IS FILE ORDER, nothing cleverer. The first rule whose wake word
and node list both match wins, so a specific rule has to sit above a catch-all.
A rule that an earlier one makes unreachable is reported in GET and PUT
/nodes/routing as a warning rather than refused, because moving a catch-all
above something is a legitimate way to switch it off for an evening.

    GET  /nodes/routing       the rules, which secret env vars are set (never
                              their values), STT/TTS URLs, warnings
    PUT  /nodes/routing       replace the whole ruleset, validated
    POST /nodes/routing/test  {"node","wake_word","text"}: skips STT, runs the
                              destination and TTS, returns the Outcome as JSON
                              and plays nothing

THESE ROUTES MUST BE INCLUDED BEFORE main.py's /nodes/{nid} routes: FastAPI
matches in registration order and GET /nodes/{nid} would otherwise take
"routing" for a node id and answer 404.
...
log = logging.getLogger("voice-nodes.router")
...
# ref -> (node id, node name), or None when no single node matches. main.py
# supplies one built on Hub.find; without it a ref is taken as both.
...
class Rule(BaseModel):
    ...
    # Node ids (MAC, colons optional) or names. Empty means every node.
    nodes: list[Annotated[str, Field(min_length=1, max_length=64)]] = Field(
        default_factory=list, max_length=64)
    destination: Destination
    # "same" (the node that heard), "none" (act, say nothing) or a node's id
    # or name, for "ask in the bedroom, answer in the kitchen".
    reply_to: str = Field(default="same", min_length=1, max_length=64)
    language: str | None = Field(default=None, pattern=LANGUAGE)
    voice: str | None = Field(default=None, max_length=64)  # unset: NODES_TTS_VOICE
    ...
    def matches(self, node_id: str, node_name: str, wake_word: str) -> bool:
        if self.wake_word != "*" and _wake_key(self.wake_word) != _wake_key(wake_word):
            return False
        if not self.nodes:
            return True
        nid, name = _mac_key(node_id), node_name.casefold()
        return any(_mac_key(n) == nid or (name and n.casefold() == name) for n in self.nodes)
    ...
    def covers(self, other: Rule) -> bool:
        ...
        if not self.nodes:
            return True
        mine = {n.casefold() for n in self.nodes} | {_mac_key(n) for n in self.nodes}
        return bool(other.nodes) and all(
            n.casefold() in mine or _mac_key(n) in mine for n in other.nodes)
...
def default_ruleset() -> RuleSet:
    """Any wake word on any node is echoed back to it: a fresh hub proves the
    whole loop (microphone, STT, TTS, speaker) before Home Assistant exists."""
...
class Rules:
    ...
    def match(self, node_id: str, node_name: str, wake_word: str) -> Rule | None:
        return next((r for r in self.rules if r.matches(node_id, node_name, wake_word)), None)
    ...
    def warnings(self, lookup: Lookup | None = None) -> list[str]:
        ...
        for j, later in enumerate(self.rules):
            ...
            if lookup and later.reply_to not in ("same", "none") and lookup(later.reply_to) is None:
                out.append(f"rule {later.id!r} replies to {later.reply_to!r}, which is not a known node")
        return out
...
class Outcome:
    ...
    reply_to: str | None = None  # a node id when it could be resolved, else the ref as written
    error: str | None = None
...
class Router:
    def __init__(self, rules: Rules, *, stt_url: str | None = None, tts_url: str | None = None,
        ...
        self.rules = rules
        env = os.environ.get
        self.stt_url = (stt_url if stt_url is not None else env("NODES_STT_URL", "")).rstrip("/")
        self.tts_url = (tts_url if tts_url is not None else env("NODES_TTS_URL", "")).rstrip("/")
        self.voice = voice or env("NODES_TTS_VOICE") or "bm_george"
        self.lookup = lookup
    ...
    async def handle(self, node_id: str, node_name: str, wake_word: str,
                     utterance_pcm16k: bytes) -> Outcome:
        out = Outcome()
        start = time.monotonic()
        try:
            rule = self._match(out, node_id, node_name, wake_word)
            if rule is None:
                return out
            ...
            await self._respond(out, rule, Request(
                node_id=node_id, node_name=node_name, wake_word=wake_word, text=text,
                audio_seconds=len(utterance_pcm16k) / 2 / MIC_RATE, language=rule.language))
        except _Failed as e:
            out.error = str(e)
        except Exception as e:  # the promise in the module docstring
            log.exception("routing failed for node %s", node_id)
            out.error = f"internal: {type(e).__name__}: {e}"
        finally:
            out.timings_ms["total"] = round((time.monotonic() - start) * 1000, 1)
            self._log(node_id, wake_word, out)
        return out

    async def handle_text(self, node: str, wake_word: str, text: str) -> Outcome:
        """The pipeline from a typed sentence: no STT, and nothing is played.
        Behind POST /nodes/routing/test."""
        out = Outcome()
        start = time.monotonic()
        found = self.lookup(node) if self.lookup else None
        node_id, node_name = found or (node, node)
        try:
            rule = self._match(out, node_id, node_name, wake_word)
            if rule is None:
                return out
            ...
            await self._respond(out, rule, Request(
                node_id=node_id, node_name=node_name, wake_word=wake_word, text=text,
                audio_seconds=0.0, language=rule.language))
        except _Failed as e:
            out.error = str(e)
        except Exception as e:
            log.exception("routing test failed for node %s", node)
            out.error = f"internal: {type(e).__name__}: {e}"
        finally:
            out.timings_ms["total"] = round((time.monotonic() - start) * 1000, 1)
        return out
    ...
    def _match(self, out: Outcome, node_id: str, node_name: str, wake_word: str) -> Rule | None:
        rule = self.rules.match(node_id, node_name, wake_word)
        if rule is None:
            out.error = f"no rule matches wake word {wake_word!r} on node {node_name or node_id!r}"
    ...
    async def _respond(self, out: Outcome, rule: Rule, req: Request) -> None:
        ...
        out.reply_to = self._target(rule, req.node_id)
    ...
    def _target(self, rule: Rule, node_id: str) -> str | None:
    ...
    async def _transcribe(self, pcm: bytes, language: str | None) -> str:
        if not self.stt_url:
            raise DestinationError("NODES_STT_URL is not set, so nothing can be transcribed")
    ...
    async def _synthesise(self, text: str, voice: str | None) -> bytes:
        if not self.tts_url:
            raise DestinationError("NODES_TTS_URL is not set, so the reply cannot be spoken")
    ...
    def _log(self, node_id: str, wake_word: str, out: Outcome) -> None:
        ...
        log.info("routing: node %s wake %r rule %s: %s, %.0f ms (%s)", node_id, wake_word,
                 out.rule_id, result, out.timings_ms.get("total", 0), stages)
        # What was said stays at DEBUG: INFO logs are kept and shipped, and a
        # household's sentences do not belong in them by default.
        log.debug("routing: node %s heard %r, replied %r", node_id, out.transcript, out.reply_text)
...
def current() -> Router:
    ...
    global _current
    if _current is None:
        _current = Router(Rules(Path(os.environ.get("NODES_DATA_DIR", "/data"))))
    return _current
...
@routes.post("/nodes/routing/test")
async def try_routing(body: TryBody, router: CurrentRouter) -> dict:
    return (await router.handle_text(body.node, body.wake_word, body.text)).as_json()
