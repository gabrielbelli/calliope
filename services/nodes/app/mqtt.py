"""Home Assistant over MQTT: every adopted node as one device, by discovery.

Optional. Nothing happens unless NODES_MQTT_URL is set (mqtt://user:pass@host:1883
or mqtts://...). Without it MqttBridge is the same object with nothing behind
it, so the hub calls it the same way either way.

    NODES_MQTT_URL      the broker; unset means no MQTT at all
    NODES_MQTT_PREFIX   Home Assistant's discovery prefix (homeassistant)
    NODES_MQTT_BASE     where the hub's own topics live (calliope/nodes)

The hub's topics, for a node <id> (its MAC without colons):
...
THE BRIDGE DECIDES NOTHING ABOUT A NODE. A command from Home Assistant becomes
a change dict for on_command, which the hub points at the same code as
PATCH /nodes/{id}, so validation, saving and telling the node happen in one
place. The bridge holds no session and never sends a node anything. That is
how the lights switch keeps its promise: it changes lights_enabled and nothing
else, so it cannot send a light pattern, least of all to a node whose lights
are off.

SWITCHES AND VOLUME SHOW THE HUB'S CONFIG, NOT THE DEVICE'S STATUS. The Nodes
tab shows the same, and the config changes the moment PATCH returns, while the
node's status only follows a round trip later: a switch fed from status would
flick back to its old position between the two. The cost is that a volume
changed with the node's own VOL buttons does not show in Home Assistant, as it
does not on the Nodes tab, because the hub does not write local changes back
into the config (and sends its own value again at the next welcome).
...
FORGETTING A NODE REMOVES ITS DEVICE. An empty retained config is how Home
Assistant deletes an entity, and the device goes with its last entity. The
list of what was published lives in memory, seeded with every adopted node at
start, so a node forgotten while the broker is down is still cleared when it
comes back. The one gap: forgotten while the broker is down AND the hub
...
One base per hub. Two hubs on one broker need different NODES_MQTT_BASE
...
log = logging.getLogger("voice-nodes.mqtt")

OnCommand = Callable[[str, dict], Awaitable[Any]]

QOS = 1
ORIGIN = {"name": "calliope voice-nodes"}
# Only for clearing: a node forgotten while offline since the hub started
...
# A node id is node_id() in main.py: lower-case hex. Anything else in that
# topic level is not ours, "bridge" included.
...
def parse_command(field: str, payload: str) -> dict | None:
    """A Home Assistant command as a PATCH /nodes/{id} change, or None.
    ...
    Assistant was never given, such as the node's name or microphone gain.
    """
...
def node_state(node: dict) -> dict:
    """The retained state for a node, as Hub.describe() gives it."""
...
def discovery_configs(node: dict, *, prefix: str, base: str,
    ...
    device = {"identifiers": [nid], "name": node.get("name") or f"node-{nid[-4:]}"}
    if node.get("model"):
        device["model"] = node["model"]
    if firmware:
        device["sw_version"] = firmware

    # Settings live in the hub and reach the node at its next welcome, so they
    # stay usable while the node is away and only go when the hub does. What
    # the node itself reports goes unavailable with the node as well.
    ...
    entities: list[tuple[str, str, dict]] = [
        ...
        ("binary_sensor", "online", {
            # The node's own availability topic, read as a state: an offline
            # node is "off" here, not "unavailable", so it can be alerted on.
...
class MqttBridge:
    """Publishes adopted nodes to Home Assistant and hands its commands back.

    publish_node() and publish_event() never wait: they note what changed and
    a background task sends it, so a slow or absent broker costs the hub
    nothing. Several changes to one node before the task runs are sent once,
    as the latest.
    """

    def __init__(self, url: str | None, prefix: str = "homeassistant",
                 base: str = "calliope/nodes", *, on_command: OnCommand | None = None,
        ...
        if url:
            try:
                ...
                # the reason the hub, and every node with it, does not start.
                self.error = str(e)
        ...
        self._dirty: dict[str, dict] = {}  # node id -> latest describe()
        self._events: deque[tuple[str, str, bytes]] = deque(maxlen=64)
        self._sent: dict[str, bytes] = {}  # retained topic -> payload, this connection
        self._published: set[str] = set()  # node ids that may have discovery on the broker
    ...
    def from_env(cls, on_command: OnCommand | None = None) -> MqttBridge:
        return cls(os.environ.get("NODES_MQTT_URL") or None,
                   os.environ.get("NODES_MQTT_PREFIX") or "homeassistant",
                   os.environ.get("NODES_MQTT_BASE") or "calliope/nodes",
    ...
    async def start(self, hub: Any, on_command: OnCommand | None = None) -> None:
        ...
        self._wake = asyncio.Event()
        self._wake.set()
        # Every adopted node may have discovery on the broker from an earlier
        # run. Counting it as published is what lets a node forgotten before
        # the first connection still be cleared by that connection.
        self._published = set(hub.store.nodes)
    ...
    def publish_node(self, node: dict) -> None:
        """Queue a node, as Hub.describe() returns it. An adopted node is
        published or updated; one that is no longer adopted is removed if it
        was published, and otherwise publishes nothing."""
        if not self.enabled or not node.get("id"):
            return
        self._dirty[node["id"]] = node
        self._wake.set()
    ...
    def publish_event(self, event: dict) -> None:
        ...
        node; the rest are ignored. So the hub may forward every event."""
        if not self.enabled:
            return
        kind, nid = event.get("type"), event.get("node")
    ...
    def _refresh(self, nid: str) -> None:
        if self._hub is None:
            return
        try:
            self.publish_node(self._hub.describe(nid))
        except Exception:
            # Called from inside the hub's own event path: a fault here must
            # cost Home Assistant one update, not a node its connection.
            log.exception("MQTT: could not describe node %s", nid)
    ...
    async def _run(self) -> None:
        try:
            ...
            log.error("NODES_MQTT_URL is set but aiomqtt is not installed; MQTT is off")
            return
    ...
    def _resync(self) -> None:
        ...
        for nid in set(self._hub.store.nodes) | self._published:
    ...
    async def _pump(self, client: Any) -> None:
        while True:
            ...
            while self._events:  # first: a press is worth more fresh than state is
                ...
                if nid in self._hub.store.nodes:
                    await client.publish(topic, payload, qos=QOS, retain=False)
            while self._dirty:
                nid = next(iter(self._dirty))
                node = self._dirty.pop(nid)
                try:
                    await self._sync(client, node)
                except BaseException:
                    # Kept for the next connection, unless something newer came in.
                    self._dirty.setdefault(nid, node)
                    raise

    async def _sync(self, client: Any, node: dict) -> None:
        nid = node["id"]
        ...
        if not node.get("adopted"):
            ...
            return
        caps_buttons = (node.get("caps") or {}).get("buttons")
        ...
        if node.get("firmware"):
            self._firmware[nid] = node["firmware"]
        root = f"{self.base}/{nid}"
        wanted = {topic: _json(cfg) for topic, cfg in discovery_configs(
            node, prefix=self.prefix, base=self.base,
            buttons=self._buttons.get(nid), firmware=self._firmware.get(nid)).items()}
        wanted[f"{root}/availability"] = b"online" if node.get("online") else b"offline"
        wanted[f"{root}/state"] = _json(node_state(node))
        ...
        for topic, payload in wanted.items():
            # A status arrives every 10 s per node; only what changed is sent.
    ...
    async def _clear(self, client: Any, nid: str) -> None:
        ...
        for memo in (self._buttons, self._firmware, self._last_wake):
            memo.pop(nid, None)
        log.info("MQTT: node %s is no longer adopted; its Home Assistant device is removed", nid)
    ...
    async def _command(self, nid: str, field: str, payload: str, retained: bool) -> None:
        if retained:
            ...
            return
        if not NID.fullmatch(nid) or nid not in self._hub.store.nodes:
            log.info("MQTT: command for %s, which is not an adopted node; ignored", nid)
            return
        ...
        if self.on_command is None:
            log.warning("MQTT: no on_command handler, so %s for node %s goes nowhere", change, nid)
            return
        try:
            ...
            log.warning("MQTT: %s for node %s failed: %s", change, nid, e)
