...
operator's own rules, set through PUT /nodes/routing, which sits behind the
same API keys (or the gateway) as adopting nodes and flashing firmware. The
legitimate targets are exactly the addresses an SSRF filter would block: Home
Assistant on the LAN, an LLM on localhost, Node-RED in the next container. A
filter would break the main use and protect nothing, because whoever can PUT
a rule can already reflash every node. The trust boundary is who may write
the rules, not what they write. Redirects are not followed, so a destination
cannot bounce a request, and its bearer token, to a host the rule never named.
"""
...
class HaConversation(_Base):
    ...
    url: Url
    token_env: str = Field(default="NODES_HA_TOKEN", pattern=ENV_NAME)
...
class Llm(_Base):
    ...
    # 401, and GET /nodes/routing shows the variable as unset.
    api_key_env: str | None = Field(default="NODES_LLM_API_KEY", pattern=ENV_NAME)
...
class Webhook(_Base):
    """POST {node, node_id, wake_word, text, audio_seconds} as JSON. A JSON
    answer with a "reply" string is spoken; any other 2xx is success with
    nothing to say, so a fire-and-forget automation needs no special reply."""

    type: Literal["webhook"]
    url: Url
    # Optional bearer for receivers that check one. Note that a URL which is
    # itself the secret (HA's /api/webhook/<id>) is shown by GET /nodes/routing
    # like any other URL; prefer a token here when the receiver allows it.
    ...
    async def call(self, client: httpx.AsyncClient, req: Request) -> str | None:
        ...
        r = await client.post(self.url, headers=headers, timeout=self.timeout, json={
            "node": req.node_name or req.node_id, "node_id": req.node_id,
