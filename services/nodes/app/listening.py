"""What one node's microphones go through on the hub, between the socket and
the router:
...
An Ear is plain synchronous CPU work with state, one per node, and it never
talks to anything: main.py runs process() in a thread pool, one call at a time
per node, and does everything a node can see or hear (lights, earcons, ducking,
the reply) on the event loop. That split is also what keeps the lights promise
checkable in one place: nothing in this file can send a node anything.
...
WITHOUT THE FRONT-END (NODES_FRONTEND=0) the mono stream is the first
...
log = logging.getLogger("voice-nodes.listening")
...
class Ear:
    def __init__(self, *, rate: int = RATE, channels: int = 4, frontend: bool = True,
        ...
        if rate != RATE:
            ...
            # a microphone here would hide a node that is misconfigured.
            raise ValueError(f"the microphones run at {rate} Hz; listening needs {RATE}")
        self.rate = rate
        self.channels = channels
        # Channel 0 is the speaker loopback on every node that has more than
        # one channel; with one channel there is nothing to cancel against.
    ...
    def stats(self) -> dict:
        """For GET /nodes. Read on the event loop while process() may be
...
def parse_wake_words(spec: str) -> dict[str, float]:
    """NODES_WAKE_WORDS: "hey_jarvis:0.5,alexa:0.6". A name with no threshold
    gets 0.5, and an empty value means no wake words at all (push-to-talk and
    /inject with ?wake_word= still work). Raises ValueError saying what is
    wrong, for the health check to repeat."""
    out: dict[str, float] = {}
