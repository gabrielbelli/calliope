...
def clean_env(monkeypatch):
    for name in ("NODES_HA_TOKEN", "NODES_LLM_API_KEY", "NODES_STT_URL", "NODES_TTS_URL",
                 "NODES_TTS_VOICE", "HOOK_TOKEN"):
...
async def test_a_fresh_hub_echoes_what_it_heard_back_to_the_node_that_heard_it(make, fake):
...
def test_the_default_ruleset_is_one_echo_rule_and_is_not_written_until_saved(tmp_path):
    ...
    assert [(r.id, r.wake_word, r.nodes, r.destination.type, r.reply_to) for r in rules.rules] == \
        [("default", "*", [], "echo", "same")]
...
def test_the_first_matching_rule_wins_so_file_order_is_the_precedence(tmp_path):
    ...
    rules.replace(RuleSet(rules=[
        rule("kitchen-jarvis", wake_word="hey_jarvis", nodes=["kitchen"]),
        rule("by-mac", wake_word="alexa", nodes=[MAC]),
...
def test_a_catch_all_above_a_specific_rule_takes_everything_and_is_reported(tmp_path):
    rules = Rules(tmp_path)
    rules.replace(RuleSet(rules=[rule("catch-all"), rule("kitchen", wake_word="alexa", nodes=["kitchen"])]))
...
async def test_no_matching_rule_is_logged_and_nothing_is_sent_anywhere(make, fake, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="voice-nodes.router"):
...
async def test_ha_is_sent_the_bearer_from_its_env_var_and_its_plain_speech_is_spoken(make, fake, monkeypatch):
    monkeypatch.setenv("NODES_HA_TOKEN", SECRET)
...
async def test_the_llm_gets_the_system_prompt_and_key_and_its_think_block_is_not_spoken(make, fake, monkeypatch):
    monkeypatch.setenv("NODES_LLM_API_KEY", LLM_KEY)
...
async def test_the_webhook_gets_the_documented_fields_and_a_json_reply_is_spoken(make, fake):
    ...
    assert json.loads(fake.sent("hook.test").content) == {
        "node": "kitchen", "node_id": NID, "wake_word": "hey_jarvis",
...
@pytest.mark.parametrize("handler, expected", [
    ...
    (lambda r: httpx.Response(401), "destination: Home Assistant refused the token in NODES_HA_TOKEN (401)"),
...
async def test_an_unset_ha_token_is_named_and_home_assistant_is_never_called(make, fake):
    ...
    assert out.error == "destination: NODES_HA_TOKEN is not set, so there is no token for Home Assistant"
...
@pytest.mark.parametrize("change, expected", [
    ({"stt_url": ""}, "stt: NODES_STT_URL is not set, so nothing can be transcribed"),
...
async def test_reply_to_another_node_is_resolved_to_its_id_through_the_lookup(make):
    nodes = {"bedroom": ("aabbccddeeff", "bedroom")}
    router = make(rule("relay", reply_to="bedroom"), lookup=nodes.get)
    out = await router.handle(NID, "kitchen", "hey_jarvis", ONE_SECOND)
    assert out.reply_to == "aabbccddeeff" and out.reply_pcm48k


def test_reply_to_a_node_that_does_not_exist_is_reported(make):
    router = make(rule("relay", reply_to="attic"), lookup={}.get)
    assert "rule 'relay' replies to 'attic', which is not a known node" in router.describe()["warnings"]
...
def test_the_ha_token_never_appears_in_get_routing_or_rules_json(api, fake, tmp_path, monkeypatch):
    ...
    put = client.put("/nodes/routing", json={"rules": [{"id": "ha", "destination": HA}]})
    got = client.get("/nodes/routing")
    tried = client.post("/nodes/routing/test", json={"node": "kitchen", "wake_word": "x", "text": "hi"})

    assert put.status_code == got.status_code == tried.status_code == 200
    assert got.json()["env"] == {"NODES_HA_TOKEN": True}
...
def test_get_routing_says_which_secrets_are_missing_without_values(api, monkeypatch):
    ...
    body = client.get("/nodes/routing").json()
    assert body["env"] == {"HOOK_TOKEN": False, "NODES_HA_TOKEN": False, "NODES_LLM_API_KEY": True}
...
def test_a_secret_pasted_into_a_rule_is_refused_rather_than_saved(api, tmp_path, destination):
    client, _ = api
    r = client.put("/nodes/routing", json={"rules": [{"id": "ha", "destination": destination}]})
...
def test_an_invalid_ruleset_is_refused_and_the_old_one_stays(api, rules):
    client, router = api
    assert client.put("/nodes/routing", json={"rules": rules}).status_code == 422
...
def test_the_test_route_skips_stt_and_returns_the_outcome_without_audio_bytes(api, fake):
    client, _ = api
    r = client.post("/nodes/routing/test", json={"node": NID, "wake_word": "hey_jarvis",
                                                 "text": "good morning"})
    body = r.json()
