...
and back to a node's speaker, lights and earcons.

Everything a node or a service would be is a fake. The node is Starlette's
test socket, with a thread that records every frame the hub sends it. STT, TTS
and webhooks are httpx.MockTransport handlers on *.test hosts, which never
resolve. The wake word model is FakeWakeWords, which fires on a marker sample
rather than on speech, so these tests need no model files; the two at the end
that do use the real openWakeWord models are skipped offline.

NODES_FRONTEND is 0 unless a test says otherwise, so the mono stream is the
first microphone as sent and a marker survives to the detector.
"""
...
def mic_frames(mono: np.ndarray, seq0: int = 0):
    """The node's frames, the same audio on all three microphones and a quiet
    loopback, which is all a front-end that is switched off looks at."""
...
class FakeWakeWords:
    ...
    def clone(self):
        return FakeWakeWords(self.thresholds)
...
class Node:
    ...
    def send_frames(self, frames) -> None:
        """As fast as the hub keeps up, and no faster. A real node sends in
        real time; a test that outran the listener would overflow its queue
        and test the dropping instead (which has a test of its own)."""
        for f in frames:
...
def env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NODES_WAKE_WORDS", "hey_jarvis:0.5")
    monkeypatch.setenv("NODES_FRONTEND", "0")
...
def test_a_dark_node_is_sent_no_lights_through_a_whole_wake_route_reply_cycle(client, events, services, plug):
    """THE USER-FACING PROMISE: a node with lights_enabled false is never sent
    "lights". Every stage that lights a ring runs here (listening, thinking,
    putting it out) and the reply is really played, so the absence below is
    the guard working and not a cycle that stopped early."""
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        assert client.patch(f"/nodes/{NID}", json={"lights_enabled": False}).status_code == 200
        assert ws.receive_json() == {"type": "config", "lights_enabled": False}
        node = plug(ws)
        node.send(utterance())
        [done] = routed(events)
        wait(node.speaker, what="the reply on the speaker")
    ...
    assert node.texts("lights") == []
    # The node was really talked to during the cycle: ducked, then unducked.
    assert [m["type"] for m in node.texts()] == ["duck", "unduck"]
...
def test_the_duck_is_lifted_before_the_reply_starts_or_the_reply_is_ducked_too(client, events, plug):
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        ...
        node.send(utterance())
        routed(events)
        wait(node.speaker, what="the reply")
    got = list(node.got)
...
def test_a_flush_stops_the_audio_already_playing_not_only_what_is_queued(client, plug):
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        client.post(f"/nodes/{NID}/tone", json={"frequency": 440, "seconds": 3})
        wait(lambda: len(node.speaker()) >= 10, what="the tone to start")
        assert client.post(f"/nodes/{NID}/flush").status_code == 204
        at_flush = len(node.speaker())
        time.sleep(0.6)
        after = len(node.speaker()) - at_flush
    assert node.texts("flush")
    assert after <= 2  # one frame may be in flight; 3 s of tone is 150 frames


def test_a_lit_node_is_shown_listening_then_thinking_then_put_out(client, events, plug):
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        ...
        node.send(utterance())
        routed(events)
        wait(lambda: node.texts("lights") and node.texts("lights")[-1]["mode"] == "off",
             what="the ring to be put out")
    modes = [m["mode"] for m in node.texts("lights")]
...
def test_a_ring_left_lit_when_lights_were_turned_off_is_put_out_when_they_return(
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        ...
        node = plug(ws)
        node.send(utterance())
        wait(lambda: any(m["mode"] == "spin" for m in node.texts("lights")), what="thinking")
        client.patch(f"/nodes/{NID}", json={"lights_enabled": False})
        services.stt_gate.set()
        routed(events)
        wait(node.speaker, what="the reply")
        dark = len(node.texts("lights"))
        client.patch(f"/nodes/{NID}", json={"lights_enabled": True})
        wait(lambda: len(node.texts("lights")) > dark, what="the ring to be put out")
    lights = node.texts("lights")
    assert [m["mode"] for m in lights] == ["pulse", "spin", "off"]
    # And the "off" came after lights were enabled again, not before.
    config = [i for i, m in enumerate(node.texts()) if m.get("type") == "config"]
    off = next(i for i, m in enumerate(node.texts()) if m.get("mode") == "off")
    assert off > config[-1]


def test_a_dark_nodes_lights_route_is_refused_rather_than_sent(client, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        client.patch(f"/nodes/{NID}", json={"lights_enabled": False})
        assert ws.receive_json()["type"] == "config"
        node = plug(ws)
        r = client.post(f"/nodes/{NID}/lights", json={"mode": "solid"})
        time.sleep(0.1)
...
def test_a_wake_word_is_published_with_its_score_and_the_command_is_routed(client, events, services, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        node.send(utterance(speech_s=1.0))
        [done] = routed(events)
        # All of it: the hub paces a reply at real time plus 300 ms, so the
        # first frame arrives long before the last.
        wait(lambda: sum(len(f) - 16 for f in node.speaker()) >= TTS_SAMPLES * 2 * 2,
             what="the whole reply")
    [wake] = of(events, "wake")
    assert (wake["node"], wake["wake_word"], wake["score"]) == (NID, "hey_jarvis", 0.9)
    ...
    assert sum(len(f) - 16 for f in node.speaker()) == TTS_SAMPLES * 2 * 2
...
def test_nothing_said_after_the_wake_word_is_not_sent_to_stt(client, events, services, plug):
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        node.send(np.concatenate((floor(0.2), wake_mark(), floor(4.5, seed=2))))
        [done] = routed(events)
...
def test_without_stt_configured_a_wake_word_is_still_published_and_routing_says_why(
    ...
    assert "NODES_STT_URL" in done["error"] and done["played"] is False


def test_a_reply_for_another_node_is_played_there_and_both_are_ducked(client, events, tmp_path, plug):
    client.put("/nodes/routing", json={"rules": [
        {"id": "ask-here-answer-there", "destination": {"type": "echo"}, "reply_to": "bedroom"}]})
    with client.websocket_connect("/nodes/ws") as ws1, client.websocket_connect("/nodes/ws") as ws2:
...
def test_a_wake_word_heard_while_an_injected_conversation_holds_the_node_is_ignored(
        client, events, services, plug):
    """The Ear only reports a wake word while it is idle, but an injected
    conversation takes the node without the Ear knowing: the hub's own
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        ...
        t = threading.Thread(target=lambda: result.update(r=client.post(
            f"/nodes/{NID}/inject", params={"play": 1}, content=wav(utterance()))))
        t.start()
...
def test_a_node_with_its_speaker_off_is_not_played_to(client, events, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        client.patch(f"/nodes/{NID}", json={"speaker_enabled": False})
        assert ws.receive_json()["type"] == "config"
        node = plug(ws)
        node.send(utterance())
        [done] = routed(events)
...
def test_a_muted_nodes_frames_are_not_listened_to(client, events, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
...
def test_the_front_end_and_wake_words_run_off_the_event_loop(client, events, monkeypatch, plug):
    ...
    other node's socket is read. The Ear runs in the thread pool, always."""
    on_loop: list[bool] = []
...
def test_a_fault_in_the_signal_code_does_not_leave_the_node_deaf(client, events, monkeypatch, plug):
    real = listening.Ear.process
    calls = {"n": 0}
...
def test_push_to_talk_starts_a_conversation_named_ptt(client, events, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        ...
        node.send(np.concatenate((voiced(1.0), floor(1.2, seed=1))))
        [done] = routed(events)
    [wake] = of(events, "wake")
...
def test_the_stop_button_cancels_the_conversation_and_flushes_the_speaker(client, events, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        node.send(np.concatenate((floor(0.2), wake_mark(), voiced(0.5))))
        wait(lambda: of(events, "wake"), what="the wake word")
        ws.send_json({"type": "button", "button": "set", "action": "press"})
        [done] = routed(events)
        wait(lambda: node.texts("flush"), what="the flush")
        # And the node listens again afterwards.
        node.send(utterance())
        routed(events, 2)
    assert done["error"] == "cancelled" and done["note"] == "cancelled"
    assert [m["type"] for m in node.texts() if m["type"] in ("duck", "unduck")][:2] == ["duck", "unduck"]


def test_a_button_mapping_is_validated_and_never_sent_to_the_node(client, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        ok = client.patch(f"/nodes/{NID}", json={"buttons": {
            "play": {"press": "ptt"}, "mode": {"release": "webhook:https://hooks.test/x"}}})
        bad = [client.patch(f"/nodes/{NID}", json={"buttons": b}) for b in (
...
def test_a_webhook_button_posts_the_press_and_still_publishes_it(client, app, events):
    ...
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        client.patch(f"/nodes/{NID}", json={"buttons": {"mode": {"press": "webhook:http://hooks.test/b"}}})
        ws.send_json({"type": "button", "button": "mode", "action": "press"})
        wait(lambda: posted, what="the webhook")
    assert posted == [{"node": "kitchen", "node_id": NID, "button": "mode", "action": "press",
                       "held_ms": None}]
...
class EarconStore:
    """The node's side of earcons.py: an empty store that takes uploads."""
...
def test_earcons_the_node_lacks_are_uploaded_at_welcome_and_the_wake_one_is_played(client, events, plug):
    store = EarconStore()
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws, **EARCON_CAPS)
        node = plug(ws, answer=store)  # answers the earcon_list that followed the welcome
        wait(lambda: len(node.texts("earcon_put")) == 3 and store.putting is None
             and len(store.files) == 3, what="three uploads")
        wait(lambda: client.get(f"/nodes/{NID}").json()["earcons"]["have"] == ["done", "error", "wake"],
             what="the hub to count them")
        node.send(utterance())
        routed(events)
    from app import earcons
    assert {k: bytes(v) for k, v in store.files.items()} == earcons.defaults()
    played = [m["id"] for m in node.texts("earcon")]
...
def test_a_node_without_earcon_support_is_never_sent_earcon_messages(client, events, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        node.send(utterance())
        routed(events)
    assert not [m for m in node.texts() if m["type"].startswith("earcon")]


# ---- inject -----------------------------------------------------------------------------------


def test_inject_runs_a_clip_through_the_whole_path_and_sends_the_node_nothing(client, events, services, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        r = client.post(f"/nodes/{NID}/inject", content=wav(utterance()))
        time.sleep(0.2)
    body = r.json()
    ...
    assert services.hosts() == ["stt.test", "tts.test"]
    assert node.got == []  # not a light, not a duck, not a sample
    assert [e.get("injected") for e in events if e["type"] in ("wake", "routed")] == [True, True]


def test_inject_with_play_speaks_the_reply_on_the_node(client, events, plug):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        node = plug(ws)
        r = client.post(f"/nodes/{NID}/inject", params={"play": 1}, content=wav(utterance()))
        wait(node.speaker, what="the reply")
    assert r.json()["played"] is True
    assert [m["type"] for m in node.texts() if m["type"] in ("duck", "unduck")] == ["duck", "unduck"]
...
def test_inject_with_a_wake_word_takes_the_whole_clip_as_the_command(client):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        r = client.post(f"/nodes/{NID}/inject", params={"wake_word": "ptt"},
...
def test_inject_that_hears_no_wake_word_routes_nothing(client, services):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        r = client.post(f"/nodes/{NID}/inject", content=wav(voiced(1.0)))
    assert r.json() == {"node": NID, "heard": None, "command": None, "outcome": None,
                        "played": False}
    assert services.hosts() == []
...
def test_inject_refuses_audio_it_would_misread(client, body, code):
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
        r = client.post(f"/nodes/{NID}/inject", content=body)
    assert r.status_code == code and "error" in r.json()


def test_inject_needs_an_adopted_node_and_play_needs_it_online(client):
    with client.websocket_connect("/nodes/ws") as ws:
        ws.send_json(hello())
        ws.receive_json()
        assert client.post(f"/nodes/{NID}/inject", content=wav(utterance())).status_code == 409
    with client.websocket_connect("/nodes/ws") as ws:
        adopt(client, ws)
    r = client.post(f"/nodes/{NID}/inject", params={"play": 1}, content=wav(utterance()))
    assert r.status_code == 409 and r.json()["error"]["code"] == "node_offline"


# ---- the rest of the wiring --------------------------------------------------------------------


def test_the_routing_routes_are_not_taken_for_a_node_called_routing(client):
    r = client.get("/nodes/routing")
...
def test_a_home_assistant_switch_goes_through_the_same_path_as_patch(client, app):
    ...
    assert client.get(f"/nodes/{NID}").json()["config"]["volume"] == 20
...
def test_a_bad_wake_word_setting_leaves_the_hub_up_and_says_why(env, fake_models, monkeypatch):
    monkeypatch.setenv("NODES_WAKE_WORDS", "hey_jarvis:loud")
    app = importlib.reload(importlib.import_module("app.main"))
    with TestClient(app.app) as c:
        body = c.get("/health").json()
    assert body["voice"]["state"] == "off" and "NODES_WAKE_WORDS" in body["voice"]["error"]
...
def test_a_signed_image_keeps_its_signature_and_the_update_carries_it(signed):
    ...
    with TestClient(app.app) as c, c.websocket_connect("/nodes/ws") as ws:
        unsigned = c.post("/nodes/firmware", params={"model": MODEL}, content=IMAGE)
        fw = c.post("/nodes/firmware", params={"model": MODEL, "version": "v2", "signature": sig},
                    content=IMAGE)
        adopt(c, ws, ota_key=kid)
        r = c.post("/nodes/ota", json={"node": NID, "sha256": fw.json()["sha256"]})
...
def test_an_unsigned_image_is_not_sent_to_a_node_that_only_takes_signed_ones(app):
    with TestClient(app.app) as c, c.websocket_connect("/nodes/ws") as ws:
        fw = c.post("/nodes/firmware", params={"model": MODEL}, content=IMAGE).json()
        adopt(c, ws, ota_key="0123456789abcdef")
        r = c.post("/nodes/ota", json={"node": NID, "sha256": fw["sha256"]}).json()
...
def test_a_malformed_signature_is_refused_before_it_is_stored(app, tmp_path):
    with TestClient(app.app) as c:
        r = c.post("/nodes/firmware", params={"model": MODEL, "signature": "bm90IGEgc2ln"},
                   content=IMAGE)
        listed = c.get("/nodes/firmware").json()["firmware"]
...
def real(env, model_dir, monkeypatch, services, tmp_path):
    monkeypatch.setenv("NODES_MODEL_DIR", str(model_dir))
    app = importlib.reload(importlib.import_module("app.main"))
    with TestClient(app.app) as c:
        route_to_fakes(app, services, tmp_path)
...
def test_a_recorded_hey_jarvis_is_heard_endpointed_and_routed_by_inject(real):
    app, c = real
    with c.websocket_connect("/nodes/ws") as ws:
        adopt(c, ws)
        r = c.post(f"/nodes/{NID}/inject", content=wav(fixture("hey_jarvis_en_us.wav")))
    body = r.json()
...
def test_hey_jarvis_through_the_front_end_on_three_microphones_is_heard_live(
    ...
    with c.websocket_connect("/nodes/ws") as ws:
        adopt(c, ws)
        node = Node(ws, backlog=lambda: app.hub.sessions[NID].mic.qsize())
        node.send_frames(struct.pack("<BBBBIQ", 1, 0, 4, 0, i, 0) + capture[off:off + FRAME].tobytes()
                         for i, off in enumerate(range(0, total - FRAME + 1, FRAME)))
        [done] = routed(events)
        stats = c.get(f"/nodes/{NID}").json()["listening"]
    [wake] = of(events, "wake")
    ...
    assert node.texts("lights")  # lit on the way, and the dark test covers the other case
