...
def node_frames(reference: np.ndarray, mics: np.ndarray) -> np.ndarray:
    """What the node sends: int16 (n, 4), loopback first."""
...
def feed(fe: FrontEnd, frames: np.ndarray, chunk: int = 320) -> np.ndarray:
    """In the node's own 20 ms frames."""
...
def test_echo_of_the_nodes_own_speaker_drops_by_at_least_15_db():
    far = speechlike(12, seed=1) * dbfs(-20)
    mics = echo(far, echo_paths()) + sensor_noise((3, len(far)))
    frames = node_frames(far, mics)
    fe = FrontEnd()
    out = feed(fe, frames)
...
def test_someone_talking_over_playback_does_not_drag_the_echo_filter_off():
    ...
    # node plays. Measured: misalignment -23.4 dB before, -23.7 dB after. A
...
def test_the_nodes_own_voice_never_steers_the_beam():
    # The loudest, most speech-like thing a node hears is its own speaker. If
    # the beam's statistics ran during playback, it would steer at itself.
    far = speechlike(12, seed=1) * dbfs(-20)
...
def test_a_talker_is_heard_over_a_noise_from_another_direction(talker_az, noise_az):
    ...
    fe = FrontEnd()
    out = traced_run(fe, node_frames(np.zeros(talk.shape[1]), talk + noise))
...
def test_direction_is_withdrawn_when_nobody_has_spoken_for_a_while():
    ...
    fe = FrontEnd()
    assert fe.direction is None
    feed(fe, node_frames(np.zeros(talk.shape[1]), talk + noise))
    assert fe.direction is not None
    quiet = plane_wave(np.random.default_rng(4).standard_normal(4 * RATE), 200) * dbfs(-50)
    feed(fe, node_frames(np.zeros(quiet.shape[1]), quiet + sensor_noise(quiet.shape)))
    assert fe.direction is None
...
def test_the_speech_flag_follows_the_talker_and_drops_after():
    ...
    frames = node_frames(np.zeros(talk.shape[1]), talk + noise)
    fe = FrontEnd()
    flags = []
...
def test_latency_is_one_block_and_within_budget():
    ...
    frames = node_frames(np.zeros(len(x)), np.stack([x, x, x]))
...
def test_no_nans_from_all_zero_or_clipped_input():
    ...
    after = feed(fe, node_frames(np.zeros(talk.shape[1]), talk + noise))
