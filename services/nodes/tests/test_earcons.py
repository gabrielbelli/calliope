...
state machine against a fake node that follows the firmware's rules
(clients/korvo-node/src/earcons.cpp). No socket and no board."""
...
class FakeNode:
    """The node's side of an upload, with the checks earcons.cpp makes: one
    upload at a time, chunks only at the expected offset, never past the
...
def converse(sync: earcons.Sync, node: FakeNode, limit: int = 1000) -> None:
    """Runs the exchange from earcon_list until neither side has anything to say."""
    msg: dict | None = node.listing()
...
def test_default_earcons_fit_what_a_node_will_store(eid):
...
def test_defaults_are_identical_every_time_so_nodes_are_not_re_sent_them():
...
def test_ids_that_are_not_safe_file_names_on_the_node_are_refused(eid):
...
def test_more_earcons_than_a_node_keeps_is_refused_up_front():
    with pytest.raises(earcons.EarconError):
...
# ---- the upload, against a fake node ----------------------------------------------


def test_a_fresh_node_receives_every_default_intact():
    node, sync = FakeNode(), earcons.Sync(earcons.defaults())
    converse(sync, node)
    assert node.files == earcons.defaults()
    assert sync.done and sync.stored == ["wake", "done", "error"] and not sync.failed


def test_a_node_that_has_them_already_is_sent_nothing():
    node = FakeNode(have=earcons.defaults())
    sync = earcons.Sync(earcons.defaults())
    assert sync.handle(node.listing()) is None
    assert sync.done and sync.stored == []


def test_only_the_stale_earcon_is_re_sent():
    have = earcons.defaults() | {"done": earcons.error()}
    node, sync = FakeNode(have=have), earcons.Sync(earcons.defaults())
    converse(sync, node)
...
def test_a_large_earcon_travels_in_several_chunks():
    ...
    node, sync = FakeNode(), earcons.Sync({"big": big})
    converse(sync, node)
    assert node.files["big"] == big
...
def test_a_node_still_formatting_its_storage_is_asked_again_not_sent_to():
    sync = earcons.Sync(earcons.defaults())
    assert sync.handle(FakeNode(ready=False).listing()) is None
    assert not sync.ready and not sync.done
    converse(sync, FakeNode())
    assert sync.done
...
def test_has_is_true_only_for_what_the_node_holds_with_the_wanted_content():
    """The hub plays an earcon only when the node can: anything else comes back
    as earcon_failed, after a silence where the chime should have been."""
