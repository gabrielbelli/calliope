...
The Hub and Store are the real ones, so the node dicts are exactly what
...
class FakeBroker:
    ...
    def node_topics(self) -> dict[str, bytes]:
...
def connect(hub: Hub, *, rssi: int = -61, caps: dict | None = None) -> SimpleNamespace:
    """A node session as far as Hub.describe() looks, which records anything
    sent to the device."""
    s = SimpleNamespace(
        ...
        adopted=NID in hub.store.nodes, sent=[])
...
async def bridge(hub, broker):
    ...
    async def on_command(nid: str, change: dict) -> None:
        # What PATCH /nodes/{id} does to the record; the real one also tells the node.
        calls.append((nid, change))
        hub.store.nodes[nid].config.update(change)
...
async def test_a_bridge_with_no_url_is_a_no_op_that_never_builds_a_client(hub):
    ...
    await b.start(hub)
    b.publish_node(hub.describe(NID))
    b.publish_event({"type": "button", "node": NID, "button": "play", "action": "press"})
...
def test_a_bad_url_turns_mqtt_off_without_putting_the_password_in_the_log(caplog):
    with caplog.at_level(logging.ERROR, logger="voice-nodes.mqtt"):
...
async def test_mqtts_connects_with_tls_and_leaves_a_last_will(hub, broker):
    ...
    assert (kw["will"].topic, kw["will"].payload, kw["will"].retain) == (
        "calliope/nodes/bridge/availability", b"offline", True)
    assert broker.retained["calliope/nodes/bridge/availability"] == b"online"
    await b.stop()
    # A clean disconnect suppresses the will, so stop() has to say it.
    assert broker.retained["calliope/nodes/bridge/availability"] == b"offline"


# ---- discovery -------------------------------------------------------------------


async def test_an_adopted_node_is_one_device_with_every_entity_retained(hub, broker, bridge):
    ...
    for topic, cfg in configs.items():
        ...
        assert cfg["availability"][0] == {"topic": "calliope/nodes/bridge/availability"}
...
async def test_discovery_payloads_have_the_shapes_home_assistant_reads(hub, broker, bridge):
    ...
    node_avail = {"topic": f"{ROOT}/availability"}

    assert c["rssi"]["device_class"] == "signal_strength"
    assert c["rssi"]["unit_of_measurement"] == "dBm"
    assert node_avail in c["rssi"]["availability"] and c["rssi"]["availability_mode"] == "all"
    ...
    # Bridge availability only: an offline node must read "off", not "unavailable".
    assert online["availability"] == [{"topic": "calliope/nodes/bridge/availability"}]
...
async def test_an_unadopted_node_publishes_nothing(hub, broker, bridge):
    s = connect(hub)
    await bridge.start(hub)
    await until(lambda: bridge.connected)
    bridge.publish_node(hub.describe(NID))
    bridge.publish_event({"type": "pending", "node": NID, "address": s.address})
    bridge.publish_event({"type": "button", "node": NID, "button": "play", "action": "press"})
...
async def test_a_status_heartbeat_republishes_state_but_not_discovery(hub, broker, bridge):
    ...
    s.status = {"rssi": -70}
    bridge.publish_event({"type": "status", "node": NID, "status": s.status})
    await until(lambda: json.loads(broker.retained[f"{ROOT}/state"])["rssi"] == -70)
    assert [t for t, *_ in broker.log[before:]] == [f"{ROOT}/state"]


async def test_an_offline_node_keeps_its_device_and_reads_offline(hub, broker, bridge):
    await adopted_and_published(hub, broker, bridge)
    configs = discovery(broker)
    del hub.sessions[NID]
    bridge.publish_event({"type": "offline", "node": NID})
...
async def test_a_forgotten_node_clears_every_config_it_published(hub, broker, bridge):
    s = await adopted_and_published(hub, broker, bridge)
    bridge.publish_event({"type": "wake", "node": NID, "wake_word": "hey_jarvis"})
    await until(lambda: f"{ROOT}/wake_word" in broker.retained)
    published = set(broker.node_topics())
    hub.store.forget(NID)
    s.adopted = False
    bridge.publish_node(hub.describe(NID))
    await until(lambda: not broker.node_topics())
    for topic in published:
        assert broker.published(topic)[-1] == b"", topic


async def test_a_node_forgotten_while_the_broker_was_down_is_cleared_on_reconnect(
        hub, broker, bridge):
    hub.store.adopt(NID, "kitchen", MODEL)
...
async def test_the_lights_switch_changes_only_lights_enabled_and_sends_the_node_nothing(
        hub, broker, bridge):
    s = await adopted_and_published(hub, broker, bridge)
    hub.store.nodes[NID].config["lights_enabled"] = False
    ...
    assert s.sent == []  # no pattern, no config: only on_command may talk to the node
...
async def test_commands_that_are_not_ours_to_obey_are_ignored(hub, broker, bridge):
    ...
    for topic, payload, retain in (
            ...
            (f"calliope/nodes/{other}/set/mic_enabled", b"OFF", False),  # not adopted
            ("calliope/nodes/bridge/set/mic_enabled", b"OFF", False),
...
async def test_a_failing_command_does_not_stop_the_bridge(hub, broker, bridge):
    ...
    async def on_command(nid, change):
        seen.append(change)
        if len(seen) == 1:
            raise RuntimeError("node 94b97e7b8be8 is not connected")
...
async def test_buttons_and_wake_words_become_home_assistant_events(hub, broker, bridge):
    await adopted_and_published(hub, broker, bridge)
    bridge.publish_event({"at": 1.0, "type": "button", "node": NID, "button": "play",
                          "action": "press", "held_ms": None})
    bridge.publish_event({"at": 1.2, "type": "button", "node": NID, "button": "play",
                          "action": "release", "held_ms": 180})
    bridge.publish_event({"type": "wake", "node": NID, "wake_word": "hey_jarvis", "score": 0.93})
...
async def test_a_lost_connection_is_retried_with_backoff_and_everything_resent(
    ...
    broker.refuse = 3
    before = len(broker.log)
    with caplog.at_level(logging.WARNING, logger="voice-nodes.mqtt"):
        broker.drop()
