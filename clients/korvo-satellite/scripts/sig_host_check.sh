#!/usr/bin/env bash
# Builds the satellite's signature check (src/ota_sig.cpp) on this machine
# against mbedTLS 2.28.7, the release in the ESP32 Arduino core 2.0.17, and runs
# it on good and bad signatures made with a throwaway key. No board is involved.
#
#   clients/korvo-satellite/scripts/sig_host_check.sh
#
...
mkdir -p "$work/gen"
python3 - "$satellite/scripts" "$work/key.pub.pem" "$work/gen/firmware_key.h" <<'EOF'
import sys
...
c++ -std=c++17 -Wall -Wextra -Werror -DFIRMWARE_SIGNING -I"$work/gen" -I"$satellite/src" \
  -I"$mbedtls/include" "$satellite/src/ota_sig.cpp" "$satellite/scripts/sig_host_check.cpp" \
