...
extern bool satellite_muted;  // main.cpp owns the privacy switch
...
static bool mic_live() {
  return connected && adopted && settings.mic_enabled && !satellite_muted;
}
...
static void send_hello() {
  ...
  // them, and it leaves them out of the welcome until it does; a hub from
  // before 2026-09-25 ignores these keys.
...
static void on_text(const char *text, size_t len) {
  ...
  if (!strcmp(type, "welcome")) {
    ...
    // for a satellite the hub has adopted.
...
#endif
  if (tls) ws.beginSslWithCA(host.c_str(), port, "/satellites/ws", CA_BUNDLE, "");
  else ws.begin(host.c_str(), port, "/satellites/ws", "");
  ws.onEvent(on_event);
