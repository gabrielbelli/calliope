...
A node built while `firmware-signing.pub.pem` is in this folder installs an
over-the-air update only when the update carries a valid signature from the
matching private key. The hub carries the signature but cannot make one, so
whoever controls the hub still cannot put their own firmware on a node.
...
openssl pkey -in ~/.config/calliope/firmware-signing.pem -pubout \
  -out clients/korvo-node/keys/firmware-signing.pub.pem
```
...
to update a node that trusts this key is over USB.
...
| `firmware-signing.pub.pem` | The build | The node it makes |
|---|---|---|
...
## Moving a node to a new key

A node trusts only the key it was built with. To change keys, build an image
with the new public key here and sign it with the old private key. The node
checks the update against the old key, installs it, and from then on trusts
the new one.

The upload script refuses that pairing on purpose, so sign and upload this one
image by hand. If the hub sets `NODES_FIRMWARE_PUBKEY`, it must still name the
old key until every node has moved.
...
curl -fsS --data-binary @"$BIN" -H 'Content-Type: application/octet-stream' \
  "$CALLIOPE_URL/nodes/firmware?model=esp32-korvo-v1.1&version=rekey&signature=$SIG"
# then POST /nodes/ota with the sha256 it returns, or use the Nodes tab
```
