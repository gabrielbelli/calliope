...
The node verifies with mbedTLS (ESP32 Arduino core 2.0.17 ships 2.28.7), which
has ECDSA on P-256 but no Ed25519: that, and nothing else, fixes the algorithm.
"""
...
class SigningError(Exception):
    """An upload that could only fail on the node. Stopping on the developer's
    machine saves a full transfer, and the node's answer is less specific."""
...
def check_p256_public_key(der: bytes) -> None:
    if len(der) != P256_SPKI_LEN or not der.startswith(P256_SPKI_PREFIX):
        raise ValueError("not an uncompressed ECDSA P-256 public key; the node can only "
                         "verify P-256 (see keys/README.md)")


def key_id(der: bytes) -> str:
    """Short name for a public key, shown by the node and the hub so a
...
def sign_for_upload(image: bytes, key_path: str, pub_path: str) -> str | None:
    """The `signature` query parameter for POST /nodes/firmware, base64url, or
    None to upload unsigned. `pub_path` is the key the build compiled in."""
    ...
    if not have_key:
        if have_pub:
            raise SigningError(
                ...
                "a public key, so the nodes running it would refuse an unsigned image.")
        print("uploading UNSIGNED: only a node built without a public key accepts it",
              file=sys.stderr)
        return None
    ...
    if have_pub:
        ...
        # Signed, so a node that requires signatures installs it, and from then on
        # accepts anything. Allowed, because that is how a node goes back to
        # development, but never quietly.
        print("WARNING: signing an image built WITHOUT a public key. A node that installs "
